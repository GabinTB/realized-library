...
def parzen_optimal_bandwidth(
    ...
    omega2_est = noise_variance(prices, timestamps, q)
...
def compute(
    ...
    kernel : str, optional
        ...
        - 'second-order': Second-order kernel
        - 'cubic': Cubic kernel
    ...
    if bandwidth is None:
        bandwidth = n**(3/5) # Rule of thumb for bandwidth
    ...
    #         gamma_h += returns[j] * returns[j - abs(h)]
    #     rk += k(h / (H + 1)) * gamma_h
    ...
    #     lag = abs(h)
    #     x = returns[lag:]
    #     y = returns[:-lag] if lag > 0 else returns
    #     gamma_h = np.dot(x, y)
    #     rk += k(h / (H + 1)) * gamma_h
